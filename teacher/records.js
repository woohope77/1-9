import {
  CFG, $, configured, db, fetchStudents, CATEGORIES, catByName, esc, photoUrl,
} from "./common.js";
...
document.title = `${className} · 활동 기록`;
$("siteTitle").textContent = `${className} 활동 기록`;
...
async function reload() {
  const { data, error } = await db.rpc("list_all_activities", { p_password: getPassword() });
